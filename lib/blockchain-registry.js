...
class BlockchainRegistry {
  ...
  static async getAccountInfo(accountName) {
    ...
    const netStakedDelegated =
      this._getTokensAmountFromString(response.total_resources.net_weight) - data.resources.net.tokens.self_delegated;
    const cpuStakedDelegated =
      this._getTokensAmountFromString(response.total_resources.cpu_weight) - data.resources.cpu.tokens.self_delegated;

    data.resources.net.tokens.delegated = netStakedDelegated;
    data.resources.cpu.tokens.delegated = cpuStakedDelegated;

    data.tokens.staked_delegated = netStakedDelegated + cpuStakedDelegated;

    if (response.refund_request) {
      const requestDatetime     = this._getRequestDateTime(response.refund_request.request_time);
      const unstakedOnDatetime  = this._getUnstakedOnDatetime(response.refund_request.request_time);

      let totalAmount = 0;

      if (response.refund_request.net_amount) {
        data.resources.net.unstaking_request.amount =
          this._getTokensAmountFromString(response.refund_request.net_amount);

        data.resources.net.unstaking_request.request_datetime = requestDatetime;
        data.resources.net.unstaking_request.unstaked_on_datetime = unstakedOnDatetime;
      ...
      if (response.refund_request.cpu_amount) {
        ...
        data.resources.cpu.unstaking_request.request_datetime     = requestDatetime;
        data.resources.cpu.unstaking_request.unstaked_on_datetime = unstakedOnDatetime;

        totalAmount += data.resources.cpu.unstaking_request.amount;
      }

      data.tokens.unstaking_request.amount                = totalAmount;
      data.tokens.unstaking_request.request_datetime      = requestDatetime;
      data.tokens.unstaking_request.unstaked_on_datetime  = unstakedOnDatetime;
    }
   ...
   * @private
   */
  static _getRequestDateTime(requestDatetime) {
   ...
   * @private
   */
  static _getUnstakedOnDatetime(requestDatetime) {
    const date = moment(requestDatetime + 'Z');
